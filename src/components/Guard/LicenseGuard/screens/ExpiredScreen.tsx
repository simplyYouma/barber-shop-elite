...
export const ExpiredScreen = ({ machineId, onReset }: Props) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-white font-outfit text-slate-900 overflow-hidden px-4">
            ...
            <div className="relative z-10 max-w-md w-full text-center animate-in fade-in zoom-in duration-700">
                <div className="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-10 relative border-2" style={{ backgroundColor: 'color-mix(in srgb, var(--yumi-primary), transparent 90%)', borderColor: 'color-mix(in srgb, var(--yumi-primary), transparent 80%)' }}>
                ...
                <button onClick={() => setShowConfirm(true)}
                    className="mx-auto bg-slate-50 p-4 rounded-2xl font-black text-[10px] text-slate-400 uppercase tracking-widest flex items-center gap-2 hover:bg-slate-100 transition-all">
